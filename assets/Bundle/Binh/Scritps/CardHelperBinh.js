var CardHelperBinh = cc.Class({
  statics: {
    getIns() {
      if (this.self == null) this.self = new CardHelperBinh();
      return this.self;
    },
  },
  ...
  is5DoiThong(cards) {
    ...
    if (this.isPair(cards[0], cards[1]) && this.isPair(cards[2], cards[3]) && this.isPair(cards[4], cards[5]) && this.isPair(cards[6], cards[7]) && this.isPair(cards[8], cards[9])) {
      if (
        this.getCard(cards[2]) == this.getCard(cards[0]) + 1 &&
  ...
  is6Doi(cards) {
    ...
    if (
      this.isPair(cards[0], cards[1]) &&
      this.isPair(cards[2], cards[3]) &&
      this.isPair(cards[4], cards[5]) &&
      this.isPair(cards[6], cards[7]) &&
      this.isPair(cards[8], cards[9]) &&
      this.isPair(cards[10], cards[11])
    ) {
      return true;
    }
    return false;
  ...
  getSanhSam(cards, isAtTo14 = false) {
    // sanh? Sâm
    let tempCards = [];
  ...
  getThungPhaSanhLon(cards) {
    ...
    listReturn.sort(function (a, b) {
      return a - b;
    });
    // cc.log("listReturn", listReturn);
    let cardDau = this.getCard(listReturn[0]);
    let cardCuoi = this.getCard(listReturn[listReturn.length - 1]);
    // cc.log("cardDau card Cuoi", cardDau, cardCuoi);
    if (listReturn.length > 0 && cardDau == 10 && cardCuoi == 14) {
      return listReturn;
    } else {
      return [];
    }
  ...
  getSanhBinh(cards) {
    ...
    if (listReturn.length > 0 && listReturn[0].length == 5) {
      listReturn[0].sort(function (a, b) {
        return b - a;
      });
      return listReturn[0];
    }
    ...
    if (listReturn2.length > 0 && listReturn2[0].length == 5) {
      listReturn2[0].sort(function (a, b) {
        return b - a;
      });
