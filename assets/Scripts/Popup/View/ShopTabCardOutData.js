...
cc.Class({
    ...
    Init() {
        ...
        for(let i = 0 , l = listInfo.length ; i < l ; i++){
            let info = listInfo[i];
            if(info.NSPType == NSP_TYPE.DATA_VINAPHONE){
                listVN.push(info);
            }else if(info.NSPType == NSP_TYPE.DATA_MOBIFONE){
                listMB.push(info)
            }else if(info.NSPType == NSP_TYPE.DATA_VIETTEL){
                listVT.push(info);
            }
        }
    ...
    creatItem(info){
        ...
        let strNph = "";
        if(nph == NSP_TYPE.DATA_VIETTEL ){
            strNph = "VT";
        }else if (nph == NSP_TYPE.DATA_VINAPHONE ){
            strNph = "VN";
        }else if(nph == NSP_TYPE.DATA_MOBIFONE ){
            strNph = "MB";
        }
        spriteNph.spriteFrame = this.listSpNphCard[nph - 1 - 5];
        listLb[0].string = strNph;
        if(info.Description)
        listLb[1].string = info.Description;
    ...
    GetNspNameByType(nspType) {
        if (nspType == NSP_TYPE.DATA_VIETTEL)
            return "Data 4G Viettel";
        else if (nspType == NSP_TYPE.DATA_VINAPHONE)
            return "Data 4G VinaPhone";
        else if (nspType == NSP_TYPE.DATA_MOBIFONE)
            return "Data 4G MobiFone";
    },
